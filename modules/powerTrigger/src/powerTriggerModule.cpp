...
#include "iCub/powerTriggerModule.h"

bool powerTriggerModule::configure(ResourceFinder &rf) {
    ...
    pThread = new powerTriggerThread(moduleName);
...
bool powerTriggerModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}


bool powerTriggerModule::respond(const Bottle& command, Bottle& reply)
{
...
bool powerTriggerModule::updateModule()
{
    return true;
}

double powerTriggerModule::getPeriod()
{
    /* module periodicity (seconds), called implicitly by myModule */
    return 1;
}

bool powerTriggerModule::close() {
